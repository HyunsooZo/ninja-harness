...
Windows native PowerShell에서는 아래 wrapper를 사용한다. 구조 검증의 source of truth는 `scripts/verify-harness-structure.py`이고, project gate 정책 구현은 `scripts/harness_lib/project_gates.py`다. Bash와 PowerShell wrapper가 같은 Python verifier/runner를 호출한다. `scripts/doctor.ps1`은 PowerShell, Python, Git, Bash/Make 준비 상태를 분리해서 보고한다. PowerShell에서 직접 실행 가능한 범위는 template/project 구조 검증과 allowlist된 `.ps1`/`.py` project gate 실행이다. shell project gate 실행 중 `.sh` script를 선택하면 `bash`가 필요하므로 Git Bash/MSYS2/WSL 또는 Linux runner를 사용한다. `make verify-org` 같은 Makefile 진입점은 계속 `bash`, `make`, POSIX 유틸리티를 전제로 한다.
...
Claude Code에서는 `.claude/settings.json`의 `PreToolUse` hook이 `Edit`/`MultiEdit`/`Write`/`NotebookEdit` 직접 수정 전에 `python3 scripts/check-evidence-gate-hook.py || python scripts/check-evidence-gate-hook.py`로 Python 본체를 실행한다. 활성 plan의 RED 증거 또는 RED 예외 없이 non-plan 파일을 직접 수정하거나, 대상 파일/범위가 active plan에 없으면 hook이 exit code 2로 차단한다. Bash wrapper는 `scripts/check-evidence-gate-hook.sh`, PowerShell wrapper는 `scripts/check-evidence-gate-hook.ps1`이다. Bash 도구로 파일을 수정하는 우회는 1차 hook의 선행 차단 범위가 아니며, `make integrity`와 리뷰로 보완한다.
