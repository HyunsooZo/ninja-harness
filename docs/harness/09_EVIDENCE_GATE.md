...
- hook command: `python3 scripts/check-evidence-gate-hook.py || python scripts/check-evidence-gate-hook.py`
...
Windows native PowerShell에서 hook을 직접 실행하거나 설정을 분리해야 하는 경우 `pwsh -NoProfile -File scripts/check-evidence-gate-hook.ps1`를 사용한다. 기본 `.claude/settings.json` command는 shell wrapper 대신 Python 본체를 직접 호출해 Bash 의존을 줄인다.
