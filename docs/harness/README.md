...
하네스 자체 Makefile/Bash 진입점은 `bash`, `make`, `python3`, `git`와 `find`/`cp`/`rm`/`mkdir`/`chmod`/`rmdir`/`sed`/`env`/`uname`/`head`/`cat`/`dirname`/`pwd` 같은 POSIX 유틸리티를 전제로 한다. Python TOML 파서는 Python 3.11+ 내장 `tomllib` 또는 Python 3.10 이하의 `tomli` fallback이 필요하다. 기본 지원 대상은 macOS, Linux/WSL, Windows의 Git Bash/MSYS/Cygwin/PowerShell 진입점이다. 조직 CI parity가 필요하면 Linux runner 또는 WSL을 우선한다.

Windows native PowerShell에서는 아래 wrapper를 사용한다. 현재 구조 검증의 source of truth는 Bash verifier이므로 PowerShell verifier도 Git for Windows/MSYS2/WSL 등에서 제공하는 `bash`를 필요로 한다. `scripts/doctor.ps1`은 PowerShell, Python, Git, Bash/Make 준비 상태를 분리해서 보고한다.
