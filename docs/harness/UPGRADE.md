...
`--changed-paths-file`에는 upstream template에서 바뀐 파일 목록을 한 줄에 하나씩 넣는다. git tag 또는 branch 기준으로 계산할 수 있으면 `--from-ref <ref>`를 대신 쓸 수 있다. `--require-clean-downstream`은 바뀐 하네스 소유 파일이 다운스트림에서 이미 달라져 있거나, `docs/harness/context/**`, `docs/harness/profiles/**`, `docs/harness/plans/**` 같은 프로젝트 소유 경로가 변경 목록에 있으면 manual merge 대상으로 실패 처리한다.
