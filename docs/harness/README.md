...
하네스 자체 스크립트는 `bash`, `make`, `python3`, `git`와 `find`/`cp`/`rm`/`mkdir`/`chmod`/`rmdir`/`sed`/`env`/`uname`/`head` 같은 POSIX 유틸리티를 전제로 한다. Python TOML 파서는 Python 3.11+ 내장 `tomllib` 또는 Python 3.10 이하의 `tomli` fallback이 필요하다. 기본 지원 대상은 macOS와 Linux/WSL이다. Git Bash/MSYS/Cygwin 계열은 POSIX 호환 shell 위에서 best-effort로 다루며, 조직 CI parity가 필요하면 WSL 또는 Linux runner를 우선한다. Windows native `cmd.exe`/PowerShell만 있는 환경은 지원 대상이 아니다.

```bash
make doctor
```

`make doctor`는 필수 스크립트 실행 권한, bash 문법, `bash`/`python3`/`make`/`git` 존재 여부, POSIX 유틸리티, Python TOML 파서 준비 상태, 지원 OS 범위를 확인한다. 이 검증은 프로젝트 build/test를 실행하지 않는다.
