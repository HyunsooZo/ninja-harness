...
Windows native PowerShell에서는 아래 wrapper를 사용한다. 구조 검증의 source of truth는 `scripts/verify-harness-structure.py`이고, Bash와 PowerShell wrapper가 같은 Python verifier를 호출한다. `scripts/doctor.ps1`은 PowerShell, Python, Git, Bash/Make 준비 상태를 분리해서 보고한다. PowerShell에서 직접 실행 가능한 범위는 template/project 구조 검증이다. `HARNESS_RUN_PROJECT_CHECKS=1`, `HARNESS_ORG_STANDARD=1`, `make verify-org`, shell project gate 실행은 여전히 allowlist된 repository shell script를 `bash`로 실행하므로 Git Bash/MSYS2/WSL 또는 Linux runner가 필요하다.
