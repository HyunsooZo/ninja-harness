...
Windows native PowerShell에서는 아래 wrapper를 사용한다. 구조 검증의 source of truth는 `scripts/verify-harness-structure.py`이고, Bash와 PowerShell wrapper가 같은 Python verifier를 호출한다. `scripts/doctor.ps1`은 PowerShell, Python, Git, Bash/Make 준비 상태를 분리해서 보고한다. Makefile과 shell project gate는 여전히 POSIX shell이 필요하지만, 구조 검증은 PowerShell에서 직접 실행할 수 있다.
